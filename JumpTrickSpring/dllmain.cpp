...
extern "C" void __declspec(dllexport) OnFrame() {
    int* p;
    static float groundpos, heightpos;
    float* maxspeed = *(float**)0x1E5E2F0;
    float* boostgauge = *(float**)0x1E5E2F0;
    byte* holdjump = reinterpret_cast<byte*>(0x1E77B54);
    static bool initialboost = false;
    static bool airspring = false;
    static bool airjump = false;
    static bool airbooston = false;
    static bool brakereverse = false;
    static bool speedbarrier = false;
    ...
    static bool slopespeedmode, landfromair = false;
    static float slopespeed, inislopespeed = 0.15f;
    static float slopespeed2 = 1.0f;
    static float multi, initmulti = 0.4f;
    static float inboostmulti, initinboostmulti = 0.2f;
    static float basespeed = 60.0f;
    static float Airbhalf, Airbfull, Airbinitial, Airbboosting = 1.0f;
    static float AF, groundaccel, revspeed = 0.0f;
    float spdashspeed, adder = 0.0f;
    static bool slide, reverseairb = false;
    static bool jump = false;
    static float preVel = 0.0f;
    static bool airrtboost = false;
    static float ringlimit, infinitelimit, chaoslimit, prevchaos, prerings = 0.0f;
    static bool infiniteflag, isdead, onboard, isjump = false;
    ...
    //Sonic::Player::CPlayer* player;   
    //player = context->m_pPlayer;
    ...
    static float tapped, tappedlimit, accelforce, accelimit, deccelimit, preVel2 = 0.0f;
    static float chargespeed, boostmulti, slopeincr, baseslopincr = 0.0f;
    static bool slopesp, xzweirdneds, lvlcam = false;
    ...
    if (context != NULL && p != NULL && maxspeed != NULL && camera != NULL) {
        ...
        //*(maxspeed + 0x53C / 4) = 90.0f;
        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_MaxVelocityFinalMax] = 145.0f;     
        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AirBoostCalibratedAirDragScale] = 0.0f;
        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AirBoostGravityRate] = 0.15f;
        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_ChaosEnergyRecoverRateByRing] = 0.0f;
        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_ChaosEnergyRecoverRateByRingBonus] = 0.0f;
        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_BoostTimeDecreaseOnWater] = 5.5f;
        //*(boostgauge + 0x5BC / 4) -= 0.5f;
        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_InAirForceBrakeRate] = 5.0f;
        ...
        if (context->m_Is2DMode == false) {
            ...
            else if (*(maxspeed + 0x53C / 4) == 70.0f) {
                basespeed = 70.0f;
                initmulti = 1.285715f;
            }
            else {
            ...
            else if (*(maxspeed + 0x53C / 4) == 35.0f) {
                basespeed = 35.0f;
                initmulti = 1.5f;
            }
            else {
        ...
        if (context->m_pPlayer->m_StateMachine.GetCurrentState() != NULL) {
            if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() != "Goal" && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() != "GoalAir" && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() != "NormalDamageDead" && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() != "NormalDamage" && context->StateFlag(eStateFlag_Pause) == false) {
                ...
                if (context->StateFlag(eStateFlag_Dead) == true)
                    isdead = true;

                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "StartDash" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "StartCrouching" || context->StateFlag(eStateFlag_ReadyGoOpened) == true || (isdead == true && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Stand")) {
                    *(boostgauge + 0x5BC / 4) = 43.3f;
                    infiniteflag = false;
                    isdead = false;
                }
                if (*(boostgauge + 0x5BC / 4) > 100.0f) {
                    *(boostgauge + 0x5BC / 4) = 100.0f;
                    prevchaos = *(boostgauge + 0x5BC / 4) - 1.0f;
                }
                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "NormalDamage" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "DamageOnRunning" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "NormalDamageAir" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "NormalDamageStandUp") {
                    *(boostgauge + 0x5BC / 4) -= 0.33f;
                    infiniteflag = false;
                    infinitelimit = 0.0f;
                }
                if (*(boostgauge + 0x5BC / 4) >= 100.0f && infiniteflag == false)//&& *(boostgauge + 0x5BC / 4) >= 100.0f)
                    infiniteflag = true;
                if (infiniteflag == true) {
                    infinitelimit += deltaTimeInSeconds;
                    if (boostmode == true)
                        isboost = true;
                    //*(boostgauge + 0x5BC / 4) = 99.0f;
                    if (*(boostgauge + 0x5BC / 4) < 97.0f) {
                        *(boostgauge + 0x5BC / 4) = 99.0f;
                        prevchaos = *(boostgauge + 0x5BC / 4);
                    }
                    context->m_ChaosEnergy -= 0.005f;

                    //*(p + 0x5B8 / 4) = 90;
                }
                if (infinitelimit >= 7.0f) {
                    *(boostgauge + 0x5BC / 4) = 99.0f;
                    prevchaos = *(boostgauge + 0x5BC / 4);
                    if (isboost == true) {
                        *(boostgauge + 0x5BC / 4) -= 33.3f;
                        isboost = false;
                    }
                    infiniteflag = false;
                    infinitelimit = 0.0f;
                ...
                //*(p + 0x5B8 / 4) = 90;
                if (chaoslimit < 2.5f)
                    chaoslimit += deltaTimeInSeconds;
                if (chaoslimit >= 2.5f) {
                    prevchaos = *(boostgauge + 0x5BC / 4);
                    chaoslimit = 0.0f;
                }
                if (prevchaos < *(boostgauge + 0x5BC / 4))
                    infinitelimit = 0.0f;
                if (ringlimit < 0.2f)
                    ringlimit += deltaTimeInSeconds;
                if (ringlimit >= 0.2f) {
                    prerings = *(p + 0x5B8 / 4);
                    ringlimit = 0.0f;
                }
                if (*(p + 0x5B8 / 4) - prerings == 10.0f) {
                    *(boostgauge + 0x5BC / 4) += 33.3f;
                    prerings = *(p + 0x5B8 / 4);
                }
                if (boostmode == true || (context->m_Grounded == false && Sonic::CInputState::GetInstance()->GetPadState().IsTapped(Sonic::eKeyState_RightTrigger) == true && airrtboost == true))
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AirBoostFirstDecrease] = 0.0f;
                else
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AirBoostFirstDecrease] = 20.1f;

                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "BoardGetOn" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "BoardFall" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "BoardJump" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "BoardJumpShort")
                    onboard = true;
                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "BoardGetOff" || (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Fall"))
                    onboard = false;

                if ((context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "JumpSpring" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "SpecialJump") && onboard == false) {
                    airspring = true;
                    airjump = true;
                    isjump = false;
                }

                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "LightSpeedDash") {
                    airspring = false;
                    airjump = false;
                }

                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Stand" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Walk") {
                    airspring = false;
                    airbooston = true;
                }
                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "StompingLand" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Sliding") {
                    airspring = false;
                    airbooston = true;
                }
                if (airspring == true) {
                    if (Sonic::CInputState::GetInstance()->GetPadState().IsTapped(Sonic::eKeyState_Y) == true || Sonic::CInputState::GetInstance()->GetPadState().IsDown(Sonic::eKeyState_Y) == true) {
                        context->ChangeState("AdlibTrick");
                        airspring = false;
                    }
                    if (Sonic::CInputState::GetInstance()->GetPadState().IsTapped(Sonic::eKeyState_LeftTrigger) == true && airjump == true) {
                        context->ChangeState("JumpShort");
                        airjump = false;
                        airbooston = false;
                        //context->m_ChaosEnergy += 5.0f;
                    }
                }
                if (Sonic::CInputState::GetInstance()->GetPadState().IsTapped(Sonic::eKeyState_X) == true && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "AdlibTrick" && airbooston == true) {
                    context->ChangeState("AirBoost");
                    airjump = false;
                }
                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "AirBoost")
                    airjump = false;
                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "AirBoost" && airbooston == false)
                    context->ChangeState("AdlibTrick");
                if (Sonic::CInputState::GetInstance()->GetPadState().IsTapped(Sonic::eKeyState_LeftTrigger) == true && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "AdlibTrick" && airjump == true) {
                    context->ChangeState("JumpShort");
                    airjump = false;
                    airspring = true;
                    airbooston = false;
                }
                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "GrindSwitch")
                    context->m_ChaosEnergy += 0.205f;
                //if(sqrtf(velocity[0] * velocity[0] + velocity[1] * velocity[1] + velocity[2] * velocity[2]) > 40.0f)
                    //context->ChangeState("MoveStop");
                /*if (slopespeedmode == false && brakereverse == false) {
                    if (context->m_Is2DMode == false)
                        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AccelerationForce] = 25.0f;
                    else
                        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AccelerationForce] = 12.5f;
                }else if(brakereverse == false){
                    if (context->m_Is2DMode == false)
                        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AccelerationForce] = 25.0f * 4.0f;
                    else
                        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AccelerationForce] = 12.5f * 4.0f;
                }*/
                ...
                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Brake" && Sonic::CInputState::GetInstance()->GetPadState().IsTapped(Sonic::eKeyState_Y) == true && Sonic::CInputState::GetInstance()->GetPadState().IsDown(Sonic::eKeyState_X) == false)
                    brakereverse = true;
                if (Sonic::CInputState::GetInstance()->GetPadState().IsTapped(Sonic::eKeyState_X) == true) {
                    brakereverse = false;

                }
                if (brakereverse == true)
                {
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_BrakeForce] = 9999.0f;
                    AF = 0.0f;
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_RotationSpeedMax] = 9000.0f;
                    context->ChangeState("Stand");
                }
                else if (slopespeedmode == true && slide == false && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() != "Drift") {
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_BrakeForce] = 40.0f;
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_RotationSpeedMax] = 860.0f;
                    if (context->m_Is2DMode == false)
                        AF = 25.0f * groundaccel;
                    else
                        AF = 12.5f * groundaccel;
                }
                else {
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_BrakeForce] = 40.0f;
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_RotationSpeedMax] = 860.0f;
                    if (context->m_Is2DMode == false) {
                        if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Drift" && spindashmode == false && spindashmode2 == false)
                            AF = 12.1f;
                        else
                            AF = 25.0f;
                    }
                ...
                if (slide == false)
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AccelerationForce] = AF;
                //if (airmode == false) {
                ...
                if (context->m_Grounded == true && airmode == true) {
                    if (velocity[1] < -5.0f) {
                        slopespeedmode = true;
                        airmode = false;
                    }
                ...
                // }
                if ((sqrtf(velocity[0] * velocity[0] + velocity[2] * velocity[2]) < basespeed / 2.0f || ((abs(Sonic::CInputState::GetInstance()->GetPadState().LeftStickHorizontal) < 0.8f && abs(Sonic::CInputState::GetInstance()->GetPadState().LeftStickVertical) < 0.8f && boostmode == false && initialboost == false))) && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Walk")
                    slopespeedmode = false;
                //if (slopespeedmode == true)
                    //slopespeed = 1.15f;
                //else
                    //slopespeed = 1.0f;
                if ((context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Jump" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Fall") && context->m_Grounded == false && velocity[1] < -15.0f)
                    landfromair = true;
                if (landfromair == true && context->m_Grounded == true && velocity[1] < 0.0f) {
                    slopespeedmode == true;
                    landfromair = false;
                }
                if (context->m_Is2DMode == false && *(maxspeed + 0x53C / 4) < 50.0f && boostmode == false)
                    *(maxspeed + 0x53C / 4) = 70.0f;
                else if (context->m_Is2DMode == true && *(maxspeed + 0x53C / 4) < 30.0f && boostmode == false)
                    *(maxspeed + 0x53C / 4) = 35.0f;
                if (Sonic::CInputState::GetInstance()->GetPadState().IsDown(Sonic::eKeyState_X) == true && *(boostgauge + 0x5BC / 4) > 0.0f) {
                    if (slopespeedmode == true) {
                        slopespeed = 0.15f;
                        if (basespeed == 60.0f && context->m_Is2DMode == false)
                            *(maxspeed + 0x53C / 4) = (basespeed * 1.4f) * (1.0f + slopespeed) * (1.0f + inboostmulti);
                        if (basespeed == 70.0f && context->m_Is2DMode == false)
                            *(maxspeed + 0x53C / 4) = (basespeed * 1.285715f) * (1.0f + slopespeed) * (1.0f + inboostmulti);
                        if (basespeed == 30.0f && context->m_Is2DMode == true)
                            *(maxspeed + 0x53C / 4) = (basespeed * 1.4f) * (1.0f + slopespeed) * (1.0f + inboostmulti);
                        if (basespeed == 35.0f && context->m_Is2DMode == true)
                            *(maxspeed + 0x53C / 4) = (basespeed * 1.5f) * (1.0f + slopespeed) * (1.0f + inboostmulti);
                    ...
                    if (context->m_Grounded == true)
                        initialboost = true;
                    //*(maxspeed + 0x53C / 4) = (basespeed * (1.0f + multi)) * (1.0f + slopespeed) * (1.0f + inboostmulti);
                }
                else {
                    boostmode = false;
                    if (slopespeedmode == true) {
                        multi = 0.0f;
                        if (initialboost == true)
                            slopespeed = 0.15f;
                        else
                            slopespeed = 0.0f;
                        if (basespeed == 60.0f && context->m_Is2DMode == false)
                            *(maxspeed + 0x53C / 4) = (basespeed) * (1.0f + slopespeed) * (1.0f + inboostmulti);
                        if (basespeed == 70.0f && context->m_Is2DMode == false)
                            *(maxspeed + 0x53C / 4) = (basespeed) * (1.0f + slopespeed) * (1.0f + inboostmulti);
                        if (basespeed == 30.0f && context->m_Is2DMode == true)
                            *(maxspeed + 0x53C / 4) = (basespeed) * (1.0f + slopespeed) * (1.0f + inboostmulti);
                ...
                if ((Vel > basespeed * initmulti) && slide == true && velocity[1] < -5.0f)
                    slopespeedmode = true;

                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Walk" && sqrtf(velocity[0] * velocity[0] + velocity[2] * velocity[2]) > basespeed - 5.0f && initialboost == false && boostmode == false && slide == false)
                    timelimit -= deltaTimeInSeconds;
                else
                    timelimit = 2.0f;

                if (timelimit <= 0.0f && initialboost == false) {
                    initialboost = true;
                    speedbarrier = true;
                    timelimit = 3.0f;
                }

                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Walk" && sqrtf(velocity[0] * velocity[0] + velocity[2] * velocity[2]) > basespeed * 1.15f && initialboost == false && boostmode == false && slide == false && context->m_Grounded == true)
                    initialboost = true;

                if (sqrtf(velocity[0] * velocity[0] + velocity[2] * velocity[2]) < 20.0f && initialboost == true) {
                    initialboost = false;
                    inboostmulti = 0.0f;
                }

                if (initialboost == true && boostmode == false && speedbarrier == true) {
                    context->ChangeState("AirBoost");
                    speedbarrier = false;
                    context->m_ChaosEnergy = prevchaos;
                }


                if (initialboost == true && boostmode == false) {
                    if (slopespeedmode == true) {
                        inboostmulti = 0.2f;
                        slopespeed = 0.15f;
                    }
                ...
                //*(maxspeed + 0x53C / 4) = (basespeed * (1.0f + multi))* (1.0f + slopespeed)* (1.0f + inboostmulti);
                //else 
                    //*(maxspeed + 0x53C / 4) = 60.0f;
                context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_InAirForceBrakeRate] = 5.0f;
                context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_InAirForceBrakeRateInLowSpd] = 99.0f;
                context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_RotationForceInAir] = 190.0f;
                if (initialboost == true)
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_InAirForceDecRate] = 1.3f;
                else
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_InAirForceDecRate] = 0.45f;
                if ((Sonic::CInputState::GetInstance()->GetPadState().LeftStickHorizontal > 0.0f || Sonic::CInputState::GetInstance()->GetPadState().LeftStickVertical > 0.0f) && (boostmode == true || initialboost == true)) {
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_BallJumpAirDragRateScale] = 0.0f;
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_InAirForceDecRate] = 0.2f;
                }
                else {
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_BallJumpAirDragRateScale] = 1.45f;

                }
                if (context->m_Grounded == false && Sonic::CInputState::GetInstance()->GetPadState().IsTapped(Sonic::eKeyState_X) == true)
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AirBoostTime] = 125.0f;


                if (context->m_Grounded == true)
                    airrtboost = true;
                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Jump" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "JumpShort")
                    isjump = true;
                if (context->m_Grounded == true)
                    isjump = false;

                if (isjump == true && context->m_Grounded == false && Sonic::CInputState::GetInstance()->GetPadState().IsTapped(Sonic::eKeyState_RightTrigger) == true && airrtboost == true && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() != "NormalDamage" && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() != "DamageOnRunning" && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() != "NormalDamageAir" && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() != "NormalDamageStandUp") {
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AirBoostTime] = 125.0f;
                    if (boostmode == false)
                        context->ChangeAnimation("Float");
                    context->ChangeState("AirBoost");
                    if (boostmode == false)
                        airrtboost = false;
                }

                if (boostmode == true && Vel > 0.3f && context->m_Grounded == false && Sonic::CInputState::GetInstance()->GetPadState().IsTapped(Sonic::eKeyState_LeftTrigger) == true) {
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AirBoostTime] = 125.0f;

                    context->SetVelocity(-context->GetHorizontalMovementDirection());


                    context->ChangeState("AirBoost");
                }

                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "AirBoost" && boostmode == true && Vel > 5.0f)
                    context->AddVelocity(context->GetHorizontalMovementDirection() * 0.5f);
                //if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Sliding")
                    //context->ChangeState("SpinSliding");

                if (sqrtf(velocity[0] * velocity[0] + velocity[2] * velocity[2]) < (basespeed / 2.0f) - 10.0f)
                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AirBoostStartVelocityRate] = Airbhalf;
                else {
                    if (initialboost == false && boostmode == false)
                        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AirBoostStartVelocityRate] = Airbfull;
                    else if (initialboost == true && boostmode == false)
                        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AirBoostStartVelocityRate] = Airbinitial;
                    else if (initialboost == true && boostmode == true)
                        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AirBoostStartVelocityRate] = Airbboosting;
                    else if (initialboost == false && boostmode == true)
                        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_AirBoostStartVelocityRate] = Airbboosting;
                }
                ...
                //if ((context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Sliding" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Drift") && velocity[1] < 0.0f && Vel < (basespeed * 1.4f) + 10.0f)
                    //context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_Gravity] = 100.0f;
                //else if ((context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Jump" || context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Fall") && context->m_Grounded == false && velocity[1] < -15.0f)
                   // context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_Gravity] = 750.0f;
                //else
                    //context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::ePlayerSpeedParameter_Gravity] = 35.0f;

                /*if (camera->m_MyCamera.m_Direction.x()> 0.0f)
                    *(p + 0x5B8 / 4) = 101;
                if (camera->m_MyCamera.m_Direction.y() > 0.0f)
                    *(p + 0x5B8 / 4) = 102;
                ...
                if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "HomingAttackAfter")
    ...
    if (context != NULL && p != NULL && maxspeed != NULL && camera != NULL && DriftSensi != NULL) {
        /**/ const uint32_t result = *(uint32_t*)((uint32_t) * (void**)((uint32_t) * &context + 0x110) + 172);
        if (!result)
            return;
        float* position = (float*)(*(uint32_t*)(result + 16) + 112);
            ...
            if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() != "Goal" && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() != "GoalAir" && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() != "NormalDamageDead" && context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() != "NormalDamage" && context->StateFlag(eStateFlag_ReadyGoOpened) == false && context->StateFlag(eStateFlag_Pause) == false) {
                ...
                if ((spindashrev == true || spindashcharge == true) && context->m_Grounded == true) {
                    ...
                    if (spindashmode == false)
                        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::eSonicParameter_DriftFrontAccelForceRateLowSpeed] = 6.7f;
                ...
                /*if (context->m_pPlayer->m_StateMachine.GetCurrentState()->GetStateName() == "Drift" && (Sonic::CInputState::GetInstance()->GetPadState().IsDown(Sonic::eKeyState_LeftStickLeft) || Sonic::CInputState::GetInstance()->GetPadState().IsDown(Sonic::eKeyState_LeftStickRight)) == false) {
                    ...
                    if (context->m_Is2DMode == false && spindashmode == false)
                        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::eSonicParameter_DriftMaxAngleVelocityLowSpeed] = 1.35f;
                    //else
                        //context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::eSonicParameter_DriftMaxAngleVelocityLowSpeed] = 5.0f;
                    if (spindashmode == false)
                        context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::eSonicParameter_DriftMaxAngleVelocity] = 2.49f;

                    context->m_spParameter->m_scpNode->m_ValueMap[Sonic::Player::eSonicParameter_DriftSideAngleForce] = 5.0f;
